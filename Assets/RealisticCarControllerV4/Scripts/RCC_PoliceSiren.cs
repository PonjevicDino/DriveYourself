        ...
        switch (sirenMode) {
            ...
            case SirenMode.On:
                ...
                if (Mathf.Approximately((int)(Time.time) % 2, 0) && Mathf.Approximately((int)(Time.time * 20) % 3, 0)) {
                    ...
                    for (int i = 0; i < redLights.Length; i++)
                        redLights[i].intensity = Mathf.Lerp(redLights[i].intensity, 600f, Time.deltaTime * 50f);
                    ...
                    if (Mathf.Approximately((int)(Time.time * 20) % 3, 0)) {
                        ...
                        for (int i = 0; i < blueLights.Length; i++)
                            blueLights[i].intensity = Mathf.Lerp(blueLights[i].intensity, 600f, Time.deltaTime * 50f);
