...
/// <summary>
/// Main input class of the RCC. Used by RCC_CarControllerV4, RCC_Camera, RCC_MobileButtons.
/// </summary>
public class RCC_InputManager : RCC_Singleton<RCC_InputManager> {
    ...
    public void GetInputs() {
        ...
        if (!Settings.mobileControllerEnabled && !LogitechGSDK.LogiIsConnected(0)) {

            // TODO: Write a custom Input Processor
            inputs.throttleInput = inputActions.Vehicle.Throttle.ReadValue<float>() * -1.0f + 1.0f;
            inputs.brakeInput = inputActions.Vehicle.Brake.ReadValue<float>() * -1.0f + 1.0f;
            inputs.steerInput = inputActions.Vehicle.Steering.ReadValue<float>() * 2.0f - 1.0f;
            ...
            inputs.clutchInput = inputActions.Vehicle.Clutch.ReadValue<float>() * -1.0f + 1.0f;
    ...
    // Logitech Wheel Inputs
    public void RCC_SteeringWheel(LogitechGSDK.DIJOYSTATE2ENGINES input)
    {
        inputs.steerInput = input.lX / 32768.0f;
        inputs.throttleInput = (input.lRx + 32768.0f) / (2.0f * 32768.0f);
        inputs.brakeInput = Mathf.Clamp((input.lRy + 32768.0f) / (2.0f * 32768.0f) * 1.5f, 0.0f, 32768.0f);
        inputs.clutchInput = (input.lRz + 32768.0f) / (2.0f * 32768.0f);
        inputs.handbrakeInput = (input.lY + 32768.0f) / (2.0f * 32768.0f);
        inputs.gearInput = logitechGear;
    }
