	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float fresnelNdotV305 = dot( NormalWS, V );
				float fresnelNode305 = ( 0.0 + 1.0 * pow( max( 1.0 - fresnelNdotV305 , 0.0001 ), 1.5 ) );
				float3 ase_reflectionWS = reflect( -V, NormalWS );
				...
				float fresnelNdotV223 = dot( NormalWS, V );
				...
				float2 uv_Flakes = packedInput.ase_texcoord6.xy * _Flakes_ST.xy + _Flakes_ST.zw;
				float clampResult228 = clamp( ( 1.0 - _FlakesRimPower ) , 0.0 , 1.0 );
				float fresnelNdotV392 = dot( NormalWS, -_DirectionalLightDatas[0].forward );
				...
				float2 uv_Specular = packedInput.ase_texcoord6.xy * _Specular_ST.xy + _Specular_ST.zw;
				
				surfaceDescription.BaseColor = ( pow( ( pow( fresnelNode305 , 1.0 ) * ( tex2D( _CloudsReflection, ( _CloudsTiling * rotator276 ) ).a * _CloudsIntensity ) ) , 0.5 ) + lerpResult221 ).rgb;
				surfaceDescription.Normal = unpack242;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = _MetallicIntensity;

				#ifdef _MATERIAL_FEATURE_SPECULAR_COLOR
				surfaceDescription.Specular = 0;
				#endif

				surfaceDescription.Emission = temp_output_86_0.rgb;
				surfaceDescription.Smoothness = ( temp_output_86_0 + ( _SmoothnessIntensity * tex2D( _Specular, uv_Specular ) ) ).r;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 uv_Flakes = packedInput.ase_texcoord4.xy * _Flakes_ST.xy + _Flakes_ST.zw;
				float clampResult228 = clamp( ( 1.0 - _FlakesRimPower ) , 0.0 , 1.0 );
				float fresnelNdotV392 = dot( ase_normalWS, -_DirectionalLightDatas[0].forward );
				float fresnelNode392 = ( 0.0 + 1.0 * pow( max( 1.0 - fresnelNdotV392 , 0.0001 ), clampResult228 ) );
				float clampResult229 = clamp( ( 1.0 - fresnelNode392 ) , 0.0 , 1.0 );
				float4 saferPower86 = abs( ( ( _FlakesIntensity * tex2D( _Flakes, uv_Flakes ) ) * clampResult229 ) );
				float4 temp_cast_3 = (_FlakesPower).xxxx;
				...
				float2 uv_Specular = packedInput.ase_texcoord4.xy * _Specular_ST.xy + _Specular_ST.zw;
		...
		Pass
		{
					...
					)
			{
				...
				float clampResult229 = clamp( ( 1.0 - fresnelNode392 ) , 0.0 , 1.0 );
				float4 saferPower86 = abs( ( ( _FlakesIntensity * tex2D( _Flakes, uv_Flakes ) ) * clampResult229 ) );
				float4 temp_cast_0 = (_FlakesPower).xxxx;
				float4 temp_output_86_0 = pow( saferPower86 , temp_cast_0 );
				float2 uv_Specular = packedInput.ase_texcoord4.xy * _Specular_ST.xy + _Specular_ST.zw;
		...
		Pass
		{
				...
				)
			{
				...
				float2 uv_Flakes = packedInput.ase_texcoord3.xy * _Flakes_ST.xy + _Flakes_ST.zw;
				...
				float4 temp_cast_0 = (_FlakesPower).xxxx;
				float4 temp_output_86_0 = pow( saferPower86 , temp_cast_0 );
				float2 uv_Specular = packedInput.ase_texcoord3.xy * _Specular_ST.xy + _Specular_ST.zw;
		...
		Pass
		{
						...
						)
			{
				...
				float2 uv_Flakes = packedInput.ase_texcoord8.xy * _Flakes_ST.xy + _Flakes_ST.zw;
				float clampResult228 = clamp( ( 1.0 - _FlakesRimPower ) , 0.0 , 1.0 );
				float fresnelNdotV392 = dot( NormalWS, -_DirectionalLightDatas[0].forward );
				float fresnelNode392 = ( 0.0 + 1.0 * pow( max( 1.0 - fresnelNdotV392 , 0.0001 ), clampResult228 ) );
				float clampResult229 = clamp( ( 1.0 - fresnelNode392 ) , 0.0 , 1.0 );
				...
				float2 uv_Specular = packedInput.ase_texcoord8.xy * _Specular_ST.xy + _Specular_ST.zw;
...
Node;AmplifyShaderEditor.SamplerNode;189;-1276.84,-1360.867;Inherit;True;Property;_Flakes;Flakes;6;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
...
WireConnection;395;0;296;0
WireConnection;395;1;242;0
WireConnection;395;4;183;0
WireConnection;395;6;86;0
WireConnection;395;7;370;0
ASEEND*/
//CHKSM=E8D74CEFA850B6AEF811E8BDD1210DF66F1E48DA