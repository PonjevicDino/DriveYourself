...
public class StudyControllerConditionD : MonoBehaviour
{
    ...
    [SerializeField] private Slider preferenceSlider;
    ...
    public void Update()
    {
        ...
        else
        {
            if (Input.GetKeyDown(KeyCode.Return))
            {
                SubmitPairwiseFeedback();
            }
        }
    }

    private void SubmitPairwiseFeedback()
    {
        float mappedScore = preferenceSlider.value;

        StudyController.AgentFeedback feedback = new StudyController.AgentFeedback
        {
            likenessScore = mappedScore,
